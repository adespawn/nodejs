...
function ensure32SignedInteger(number, name) {
    ...
    if (number > maxInt32) {
        throw new TypeError(`${name} was expected to be 32bit integer, but it's larger than allowed (${number} > ${maxInt32})`)
    }
}
...
function ensure64SignedInteger(number, name) {
    if (number < minInt64) {
        throw new TypeError(`${name} was expected to be 64bit integer, but it's smaller than allowed (${number} < ${minInt64})`)
    }
    if (number > maxInt64) {
        throw new TypeError(`${name} was expected to be 64bit integer, but it's larger than allowed (${number} > ${maxInt64})`)
    }
}
