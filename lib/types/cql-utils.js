...
/**
 * Guess the type, and if the type cannot be guessed, throws an error.
 * @param {any} value
 * @returns {rust.ComplexType} type guess, converted to ``ComplexType`` object
 */
function guessTypeChecked(value) {
    let type = rust.convertHint(guessType(value));
    if (!type) {
        throw new TypeError(
 ...
 * @returns {Array<[rust.QueryParameterWrapper, rust.QueryParameterWrapper]>}
 */
...
function convertHints(hints) {
    ...
    for (const hint of hints) {
        if (hint) {
            ...
            result.push(rust.convertHint(objectHint));
        } else {
            result.push(null);
        }
    }
