...
const executeConcurrent =
    require("../../../../lib/concurrent").executeConcurrent;
...
describe("executeConcurrent()", function () {
        ...
        it("should not store individual results", () => {
            ...
            return executeConcurrent(client, insertQuery1, values)
                .then((result) => {
                    ...
                    assert.throws(
                        () => result.resultItems,
                        /can not be accessed when collectResults is set to false/,
                    );
                })
                .then(() => validateInserted(client, id, values.length));
        });
    ...
    describe("with fixed query and a stream", () => {
        it("should support a transformed text file as input", () => {
            ...
            return executeConcurrent(client, insertQuery1, transformStream, {
                concurrencyLevel: 10,
            })
                .then((result) => {
                    assert.strictEqual(
        ...
        it("should reject the promise when there is an execution error", () => {
            ...
            const transformStream = fsStream.pipe(
                new LineTransform(Uuid.random(), { fixedValues }),
            );
            let error;
        ...
        it("should resolve the promise when there is an execution error and raiseOnFirstError is false", () => {
            ...
            const transformStream = fsStream.pipe(
                new LineTransform(id, { fixedValues }),
            );
        ...
        it("should reject the promise when there is a read error", () =>
            ...
            Promise.all(
                [true, false].map((raiseOnFirstError) => {
                    const fsStream = fs.createReadStream(__filename);
                    const transformStream = fsStream.pipe(
                        new LineTransform(Uuid.random(), { failAtIndex: 15 }),
                    );
                    let error;
        ...
        it("should execute the different queries", () => {
            ...
            return executeConcurrent(client, queryAndParameters)
                .then((result) => {
                    ...
                    assert.throws(
                        () => result.resultItems,
                        /resultItems can not be accessed/,
                    );
                })
...
function getParameterValues(id, length) {
    return Array.from(new Array(length).keys()).map((x) => [
        id,
        x,
        x.toString(),
    ]);
...
function validateInserted(client, id, totalExpected) {
    return client
        .execute(countQuery1, [id])
        .then((rs) =>
            assert.equal(rs.first()["total"].toString(), totalExpected),
        );
}
...
class LineTransform extends Transform {
    ...
    _transform(chunk, encoding, callback) {
        ...
        for (let i = 0; i < parts.length; i++) {
            ...
            const values = this._fixedValues.get(index) || [
                this._id,
                index,
                parts[i],
            ];
